        ...
        **init_config,  # Add init_from_nemo_model or init_from_pretrained_model
        ...
        'exp_manager': {
            ...
            'checkpoint_callback_params': {
                ...
                'save_last': not config['training'].get('disable_checkpointing', False),
            },
...
def train(config_path: str):
    ...
    try:
        ...
        # Step 3.55: Disable spec augmentation for overfit sanity checks
        if config['training'].get('disable_spec_augment', False):
            if hasattr(asr_model, 'spec_augmentation'):
                asr_model.spec_augmentation = None
            if hasattr(asr_model, 'cfg') and hasattr(asr_model.cfg, 'spec_augment'):
                asr_model.cfg.spec_augment = None
            logging.info("  🚫 Disabled spec augmentation (overfit mode)")
        
        # Step 3.6: Freeze encoder if requested (for new tokenizer training)
        if config['training'].get('freeze_encoder', False):
            logging.info("\n" + "="*70)
            logging.info("Step 3.6: Freezing Encoder")
            logging.info("="*70)
            ...
            if hasattr(asr_model, 'encoder'):
                ...
                logging.info(f"  ❄️ Frozen encoder: {encoder_params:,} parameters")
            
            # Also freeze preprocessor if present
            if hasattr(asr_model, 'preprocessor'):
                for param in asr_model.preprocessor.parameters():
                    param.requires_grad = False
                logging.info(f"  ❄️ Frozen preprocessor")
            ...
            logging.info(f"  📊 Trainable: {trainable:,} / {total:,} ({100*trainable/total:.1f}%)")
            logging.info("  💡 Only decoder and joint layers will be trained")
        ...
        use_8bit = config['training'].get('optimizer') in ['adamw_8bit', 'adamw8bit']
        ...
        if use_8bit and BITSANDBYTES_AVAILABLE:
            ...
            try:
                ...
                # Get only trainable parameters for 8-bit optimizer
                trainable_params = [p for p in asr_model.parameters() if p.requires_grad]
                logging.info(f"  Creating 8-bit optimizer for {len(trainable_params)} param groups")
                
                # Now replace the optimizer with 8-bit version (only trainable params!)
        ...
        # Verify freeze is still in effect after optimizer setup
        if config['training'].get('freeze_encoder', False):
            encoder_frozen = all(not p.requires_grad for p in asr_model.encoder.parameters())
            trainable_after = sum(p.numel() for p in asr_model.parameters() if p.requires_grad)
            if encoder_frozen:
                logging.info(f"  ✓ Verified: Encoder still frozen, {trainable_after:,} trainable params")
            else:
                logging.error("  ❌ ERROR: Encoder became unfrozen after optimizer setup!")
                # Re-freeze it
                for param in asr_model.encoder.parameters():
                    param.requires_grad = False
                logging.info("  🔧 Re-froze encoder")
